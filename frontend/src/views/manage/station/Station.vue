...
export default {
  name: 'station',
  components: {stationAdd, stationEdit, RangeDate},
  data () {
    return {
      advanced: false,
      stationAdd: {
  ...
  methods: {
    handlestationViewClose () {
      this.stationView.visiable = false
    },
    handlestationViewOpen (row) {
      this.stationView.data = row
    ...
    editStatus (row, status) {
      this.$post('/cos/station-info/account/status', { stationId: row.id, status }).then((r) => {
